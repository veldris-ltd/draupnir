...
REGISTER: dict[str, Entry] = {
    ...
    "AC-N8": Entry(
        IMPLEMENTED,
        "90 per cent on the unit stage, 87 on the contract stage and 77 on the "
        "integration stage, over every module the distribution ships -- the set is "
        "derived from the tree, so a module under no floor fails the build and an "
        "exclusion is a decision recorded with its reason (RF-20). Each stage also "
        "refuses a run in which a named target measured nothing (RF-19). Every "
        "transition in SAD 6.1 is exercised by name. The percentages read lower "
        "than before those two findings and the coverage is larger: the unit stage "
        "went from 4,788 statements covered to 6,661, and the integration stage "
        "from 788 to 2,313.",
        "make test-unit",
    ),
    "AC-N9": Entry(
