...
REGISTER: dict[str, Entry] = {
    ...
    "AC-N8": Entry(
        IMPLEMENTED,
        "91 per cent on the unit stage, 87 on the contract stage and 81 on the "
        "integration stage, and every transition in SAD 6.1 exercised by name. Each "
        "stage now refuses a run in which a named target measured nothing: eleven of "
        "them were doing exactly that, so the figures were computed over a smaller "
